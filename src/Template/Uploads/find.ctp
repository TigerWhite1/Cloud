...
<table>
	...
	<?php foreach ($files as $key => $value): ?>
		<?php if ($value->rule == 0 && $value->folder != $username || !empty($value->file)) { ?>
		<tr>
			<td><?= $value->folder ?></td>
			<td><?php 
				...
				<td>
					...
					?>
					<?= $this->Html->link('Modifier fichier', ['action' => 'edit', $value->id]) ?>
					<?= $this->Form->postLink(
						' | Supprimer fichier',
					...
					<?php }else { ?>
					<td>
						<?= $this->Html->link('Modifier dossier', ['action' => 'editfolder', $value->id]) ?>
						<?= $this->Form->postLink(
							' | Supprimer dossier',
							['action' => 'delete', $value->id],
							['confirm' => 'Etes-vous sûr?']); ?>
